  ...
  if (show || mouseIn) {
    return (
      ...
      >
        <SearchInfoTitle>
          热门搜索
          <SearchInfoSwitch onClick={() => switchList(spinIcon)}>
            <i
              ref={(icon) => {
                spinIcon = icon;
              }}
...
const Header = (props: commonInterface.IHeaderProps) => {
  const {
    logout,
    login,
    mouseIn,
...
const mapDispatch = (dispatch: any) => {
  return {
    ...
    switchList(spinIcon: any) {
      let rotate = parseInt(spinIcon.style.transform.replace(/[^0-9]/gi, ""));
      if (!rotate) {
        rotate = 0;
      }
